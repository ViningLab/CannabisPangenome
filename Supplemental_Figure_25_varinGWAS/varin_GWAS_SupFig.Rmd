...
#p <- p + xlab("Chr 7, 290.8 Kbp")
#p <- p + ggtitle(my_locus)
#p <- p + xlab("Chr X, 12.5 Mbp")
p <- p + xlab("Chr X, 44.5 Mbp")
#p <- p + ggtitle( paste(my_locus, my_alleles, sep = ": ") )
#p <- p + ggtitle( "CHROM 7, POS 290,913" )
#p <- p + annotate( geom = "text", x = 0, y = 60, label = "Chromosome 7,\nposition 290,913", size=4.0)
#
...
ga1 <- ggarrange( p_pheno, p_locus1, 
                  nrow = 1, ncol = 2, 
                  vjust = 0.3,
                  labels = c("B", "C"))
#ga1
...
p_locus2 <- p_locus2 + theme(axis.title.y = element_text(angle = 90)) + ylab("Varin Content")


ga2 <- ggarrange( p_locus2, p_chr9_19, p_chrX_12, 
                  nrow = 1, ncol = 3, 
                  #label.y = 1.0,
                  vjust = 0.3,
                  labels = c("D", "E", "F"))
...
ggarrange( p_mh, ga1, ga2,
           nrow = 3, ncol = 1,
           labels = c("A", ""),
           #label.y = 1,
           #vjust = 2.5,
           widths = 1, heights = c(0.4, 0.3, 0.3))
...
```{r}
gff <- read.table("/media/knausb/E737-9B48/releases/scaffolded/EH23b/EH23b.primary_high_confidence.gff3.gz", sep = "\t", quote = "\"")
#gff[1:3, 1:8]
mRNA <- gff[ gff[ ,3] == "mRNA" , ]
#mRNA[1:3,]
mRNA[,10] <- unlist(lapply(strsplit(mRNA[, 9], split = ";"), function(x){ grep("eggnog_free_text_desc=", x, value = TRUE) }))
mRNA[,10] <- sub("eggnog_free_text_desc=", "", mRNA[,10])
mRNA[,11] <- unlist(lapply(strsplit(mRNA[, 9], split = ";"), function(x){ grep("^ID=", x, value = TRUE) }))
mRNA[,11] <- sub("^ID=", "", mRNA[,11])
...
for(i in 1:4){
  my_chrom <- mRNA[ mRNA[, 1] == gwas_top$Chr[i], ]
  if( any(i == c(2, 3)) ){
    my_buffer <- 1e5
  } else {
    my_buffer <- my_buf
  }
  my_win <- my_chrom[ my_chrom[, 4] >= gwas_top$Pos[i] - my_buffer & my_chrom[, 5] <= gwas_top$Pos[i] + my_buffer, ]
  
  print(knitr::kable(gwas_top[i, c(2:6, 8, 10)], row.names = FALSE, digits=3, format.args = list(big.mark=",")))
  print(knitr::kable(my_win[, c(1,4, 10)], row.names = FALSE))
}
