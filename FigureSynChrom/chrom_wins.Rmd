...
i <- 1
ncol(chrom_l[[i]])
is.na(chrom_l[[i]][ chrom_l[[i]] == 0 ]) <- TRUE
heatmap.bp(chrom_l[[i]])
title( main = paste("Chromosome:", names(chrom_l)[i]) )
```
...
i <- 3
#is.na(chrom_l[[i]][ chrom_l[[i]] == 0 ]) <- TRUE
heatmap.bp(chrom_l[[i]][ , colnames(chrom_l[[i]]) != "STHa"])
title( main = paste("Chromosome:", names(chrom_l)[i]) )
```
...
barplot(chrom_l[[i]][win_num, 10:30], las = 3)
title( ylab = "Genes")
title( main = paste("Chromosome:", names(chrom_l)[i], ", window", win_num - 1, "to", win_num , "Mbp" ) )
```
