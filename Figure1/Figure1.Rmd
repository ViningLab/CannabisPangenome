...
```{r}
chrlen <- read.csv("../FigureS1/chrom_lengths.csv", row.names = 1)
#chrlens[1:3, ]
```
...
```{r, fig.width=10, fig.height=10, message=FALSE, fig.cap="**Figure 1. Genomic architecture of** ***Cannabis sativa*** **strain ERBxHO40_23, haplotype a.** (Panel A) Circular representation of the ERBxHO40_23 haplotype A geneome. (A) Gene density, the background of each panel is colored by GC content (legend in lower left corner). (B) Density of the motif 'CG' (also referred to as 'CpG', a motif associated with methylation). (C) *Cannabis* centromere motif density (370bp motif, blastn hits). (Panel B) Ideogram of the hemp genome. Chromosomes are named according to the CBDRx assembly (NCBI: GCF_900626175.2_cs10). Each autosomal ideogram summarized the length of 69 autosomal assemblies. The ideogram for the X chromosome summarizes the length of 65 X chromosomal assemblies. The Y chromosomal ideogram summarizes the length of 4 Y chromosomal assemblies. (Panel C) A Y chromosome panel? The strain ERBxHO40 resulted from a cross between strains ERB (type III or CBD dominant) and HO40 (type I or THC dominant; a genetically female plant (XX) that was chemically masculinized to produce pollen flowers to facilitate the cross)."}
